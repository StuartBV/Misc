...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[InvoiceMessageData_Get_Invoice]
@QueueID int, 
...
begin
	select case ds.eventType when 0 then 'Y' else 'N' end DataToSend, 
		...
		from
		MessageQueue.dbo.[Queue] q join Data_Standing ds on ds.messageid=q.[id] and ds.MessageType='I'
		...
		left join CPLInvoiceNumber n1 on n1.sourcekey=ds.sourcekey and n1.data=ds.Data and n1.SupplierID in (6650,6573) and q.[id]=n1.messageID
	where q.[id]=@QueueID
	...
	select 'OR' ListOwner, '1' ListNo, '1050030C' ItemTypeCode,
		'XS' SupplierItemReference, 'Excess' ItemDetailDescription, ds.sourcekey,
		0 - ds.excess * @neg NetPrice,
		1 VatRate, 'XS', 'NU10 001' OperationCode, 1 NoOf,
		0 - ds.excess * @neg UnitPrice, 
		'False' VatApplicableInd, 0 VatAmount, 0 VatPercent, 
		convert(char(10), isnull(ds.InvoiceSent, @dt), 120) RateEffectiveDate, 
		isnull(m.eCode, 'unknown') eSupplierCode
	from
	MessageQueue.dbo.[Queue] q join Data_Standing ds on ds.messageid=q.[id] and ds.MessageType='I' and isnull(ds.Excess, 0) > 0
	join eCodeMappings_Ranked(@QueueID, null) m on m.messageid=ds.messageid
