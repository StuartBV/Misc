...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[GetClaimsEntered] as

select cc.ClaimNoPrefix + cast(p.ICardsID as varchar) as iCardsID, p.[Status], p.InsurancePolicyNo, p.InsuranceClaimNo, p.IValRef,                       
...
where convert(varchar(12),p.createdate,113)=convert(varchar(12),getdate(),113) and p.ivalref is null
GO
