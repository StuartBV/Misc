...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Rep_OptionsDailyThroughput] 
@headers tinyint=0
as
set transaction isolation level read uncommitted
set nocount on
declare @qtytotal int, @valuetotal money, @valuetotal2 money, @start datetime=getdate()-1, @end datetime=getdate()

select @qtytotal=count(*), @valuetotal=sum(cardvalue), @valuetotal2=sum(case when [type]='M' then cardvalue else 0 end)
from Transactions
...
from (
	select 'A' as seq, 'iVal Options Card Daily Activity Report for '+isnull(convert(char(10),getdate()-1,103),'') as OptionsRef, '' as [type], '' as Surname, '' as PostCode, 
	...
	select 'D' as seq,
		...
		case when p.cancelreason is not null then 'Yes - '+sys2.[description] else '' end as Cancelled, t.createdate
	...
	'' as BatchFileUploaded, '' as UploadStatus, '' as AuthorisationRequired, '' as Cancelled, '' as createdate
	from (
		select 'G' seq, 'Card [type] (New Cards Only)' CardType, 'Qty' Qty, 'Total Value' value
		union all 
		select 'H' seq, sl.[description] as CardType,cast(isnull(x.qty,0) as varchar) Qty,cast(isnull(x.value,0) as varchar) value
		from syslookup sl
		left join cards c on sl.code=c.cardtype and sl.TableName='CardType'
		left join transactions t on t.cardid=c.ID
		left join (
			select sl.[description] as [CardType], count(*) Qty, sum(t.cardvalue) value
			from transactions t
	...
	union all	
	select 'I' as seq, 'TOTAL' as OptionsRef, cast(@qtytotal as varchar) as [type], cast(@valuetotal2 as varchar) as Surname, '' as PostCode, '' as SchemeName,
	'' as PolicyNumber, '' as ClaimNumber, '' as IncidentDate, '' as CardType, '' as TransactionType, '' as CardValue, '' as RequestDate,
	'' as BatchFileUploaded, '' as UploadStatus, '' as AuthorisationRequired, '' as Cancelled, '' as createdate
)x
