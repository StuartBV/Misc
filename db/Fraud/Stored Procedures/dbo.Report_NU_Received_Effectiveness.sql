...
CREATE procedure [dbo].[Report_NU_Received_Effectiveness]
@from varchar(20),
@to varchar(20)
as
set nocount on
...
and c.channel IN ('ival','nuibrad')
and not exists (
	SELECT * FROM ppd3..ICE_BusinessRules_Log WHERE claimid=c.OriginClaimID and roletype=3 
)
...
join claims c with (nolock) on f.claimid=c.claimid
join ClaimProperties cp with (nolock) on c.ClaimID = cp.ClaimID
join ppd3..ICE_BusinessRules_Log l on l.ClaimID=c.OriginClaimID
where f.CreateDate between @fd and @td
and l.RoleType=3
and f.originatingsys='PPD3'
and c.channel IN ('ival','nuibrad')
--and f.Manual=0
--and isnull(cp.FraudIndicator,'') in ('','0')
group by c.channel,l.Description,f.ReasonClosed,l.itemid
...
join claims c with (nolock) on f.claimid=c.claimid
join ClaimProperties cp with (nolock) on c.ClaimID = cp.ClaimID
join ppd3..ICE_ClaimProducts ip (nolock) on c.OriginClaimID=ip.claimid 
join ppd3..ICE_Categories ic (nolock) on ip.catid=ic.id
...
from #sf_log l
join (
	select indicator,sum(received) [vol] from #sf_log where received>0 group by indicator
) s on l.indicator = s.indicator
where l.received>0
order by 1
...
from #sf_log l
join (
	select indicator,sum(closed) [vol] from #sf_log where closed>0 group by indicator
) s on l.indicator = s.indicator
join (
	select indicator,reasonclosed,sum(closed) [vol] from #sf_log where closed>0 group by indicator,reasonclosed
) i on l.indicator = i.indicator and l.reasonclosed=i.reasonclosed
where l.closed>0
...
from #br_log l
join (
	select channel,businessrule,sum(received) [vol] from #br_log where received>0 group by channel,businessrule
) s on l.channel = s.channel and l.businessrule = s.businessrule
join (
	select channel,sum(received) [total] from #br_log where received>0 group by channel
)t on l.channel = t.channel
where l.received>0
order by 1,2
...
from #br_log l
join (
	select channel,businessrule,sum(closed) [vol] from #br_log where closed>0 group by channel,businessrule
) s on l.channel = s.channel and l.businessrule = s.businessrule
join (
	select channel,businessrule,reasonclosed,sum(closed) [vol] from #br_log where closed>0 group by channel,businessrule,reasonclosed
) i on l.channel = i.channel and l.businessrule = i.businessrule and l.reasonclosed=i.reasonclosed
where l.closed>0
group by  l.channel,l.businessrule,s.vol,i.reasonclosed,i.vol, round((cast(i.vol as decimal(9,2))/cast(@total_received as decimal(9,2)))*100,2)
order by 1,2
...
from #cat_log l
join (
	select channel,category,sum(closed) [vol] from #cat_log where closed>0 group by channel,category
) s on l.channel = s.channel and l.category = s.category
join (
	select channel,category,reasonclosed,sum(closed) [vol] from #cat_log where closed>0 group by channel,category,reasonclosed
) i on l.channel = i.channel and l.category = i.category and l.reasonclosed=i.reasonclosed
where l.closed>0
