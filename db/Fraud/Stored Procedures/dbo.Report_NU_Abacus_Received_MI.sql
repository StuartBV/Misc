...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Report_NU_Abacus_Received_MI]
@from varchar(10),
@to varchar(10)
as
set nocount on
...
from fraud.dbo.claims c
join fraud.dbo.fraud f on f.claimid=c.claimid
join fraud.dbo.customers cu on cu.[ID]=c.[CustID]
left join fraud.dbo.ClaimProperties cp on cp.claimid=c.claimid
left join fraud.dbo.sysLookup sys on sys.tablename='FraudStatus' and sys.code=f.[status]
left join fraud.dbo.sysLookup sys2 on sys2.[TableName]='Risk' and sys2.code=f.risk
left join ppd3.dbo.claims pc on c.OriginClaimID=pc.claimid and pc.Channel='af'
where f.CreateDate between @fd and @td and f.originatingsys='FNOL' and c.Channel='abacus'
order by f.fin
GO
