...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[Report_NU_Rule_Effectiveness]
@from varchar(20),
@to varchar(20)
as
set nocount on
...
s.vol, case when @total_received = 0 then 0 else round((cast(s.vol as decimal(9,2))/cast(@total_received as decimal(9,2)))*100,2) end [per]
from #br_log l
join (
...
where l.closed>0
group by 'SF '+cast(l.indicator as varchar), s.vol, i.reasonclosed, i.vol , case when @total_received = 0 then 0 else round((cast(i.vol as decimal(9,2))/cast(@total_received as decimal(9,2)))*100,2) end
order by 1,2
...
s.vol, 
case when @total_received = 0 then 0 else round((cast(s.vol as decimal(9,2))/cast(@total_received as decimal(9,2)))*100,2) end per
from #sf_log l
join (
