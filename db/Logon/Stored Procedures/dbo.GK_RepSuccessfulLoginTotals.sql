...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[GK_RepSuccessfulLoginTotals] 
@from varchar(10),
@to varchar(10),
@cid int
as
...
select x.[description] + ' ' + cast(x.[Year]  as varchar) as [date],x.Client,x.[User],x.[Total Successful Logins], 'Success Login Totals' as Repname
from (
	...
	count(*) as [Total Successful Logins]
	from authlog au join userdata ud  on ud.username=au.userid
	...
	where au.code='0' -- A successful login
